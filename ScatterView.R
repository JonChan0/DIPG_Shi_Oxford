### Edited script from MAGeCKFlute to tailor to my purposes.
# Sources ScatterView() and Cutof
...
#'
#' @import ggplot2 ggrepel showtext
#' @export
#'
#'
...
ScatterView<-function(data, x = "x", y = "y", label = 0,
                      ...
                      main = NULL, xlab = x, ylab = y, legend.position = "none", depmap=TRUE, ...){
  requireNamespace("ggplot2", quietly=TRUE) || stop("need ggplot package")
  requireNamespace("ggrepel", quietly=TRUE) || stop("need ggrepel package")
  requireNamespace("showtext", quietly=TRUE) || stop("need showtext package")
  ...
  avail_groups = c("topleft", "topright", "bottomleft", "bottomright",
                   ...
                   "top", "mid", "bottom", "left", "center", "right", "none", 'CommonEssential')
    
  ## Select the colors
  mycolour = c('#66c2a5', '#fc8d62','#8da0cb','#e78ac3',
               '#a6d854','#ffd92f','#e5c494','#b3b3b3','#66c2a5', 
               '#fc8d62','#8da0cb','#e78ac3','#66c2a5','#ffd92f','#fc8d62','#b3b3b3', 'green') #Edited to give Set2 colours consistent with previous plots
  ...
  #Addendum by JC to add green labels for only the Depmap ones via changing up the groupname from the common essential ones 
  if (depmap == TRUE){
    for (i in seq(1,length(gg$group))){
      if(isTRUE(gg$common_essential[[i]]) && gg$Label != ''){
        gg$group[[i]] <- 'CommonEssential'
      } else if (isFALSE(gg$common_essential[[i]])){
        gg$group[[i]] <- gg$group[[i]]
      }
    }
  }
  ...
  p = p + labs(x=xlab, y = ylab, color = NULL)
  #p = p + theme_bw(base_size = 12)
  p = p + theme_classic() +
    theme(text=element_text(family='futura_bkt'))
  p = p + theme(legend.position = legend.position)  #Addendum by JC = setting the theme for font
  
  #Addendum for comparative plot
  
  if (auto_cut_diag==TRUE){
    p =p + geom_vline(xintercept = 0, linetype = 'dotted', col='black')+
      geom_hline(yintercept=0, linetype = 'dotted', col='black')
  }
  
  #Addendum by JC to add thresholding lines for top 100
  if (x == 'Rank'){
    #Defining the thresholds for top 100 hits for both +ve and -ve selection
    threshold_pos_y <- data$Diff[data$Rank==100]
    threshold_neg_y <- data$Diff[data$Rank == nrow(data)-100]
    
    p = p + geom_hline(aes(linetype='dashed'), yintercept = threshold_pos_y, col= 'black', alpha = 0.1)+
      geom_hline(aes(linetype = 'dashed'),yintercept = threshold_neg_y, col= 'black', alpha = 0.1) +
      scale_linetype_manual(name='Top 100 Rank Threshold',values=c('dashed'))
  }
 
  #Addendum by JC to add Depmap Common Essential Gene
  if (depmap == TRUE){
    if (x == 'Rank'){
      p = p + labs(title=str_wrap(paste(main,'with DepMap common essential genes (green) distributed throughout'), width=75))
      p = p + geom_point(col='green', alpha=0.1, shape=16,size = 0.5, data=filter(p$data, common_essential==TRUE))
    } else if (auto_cut_diag==TRUE){
      p = p + labs(title=str_wrap(paste(main,'with similar selection across both for the majority of DepMap common essential genes (green)'), width=75))
      p = p + scale_x_continuous(expand=expansion(add=0.5))+
        scale_y_continuous(expand=expansion(add=0.5))
      p = p + geom_point(col='green', alpha=0.5, shape=16,size = 0.5, data=filter(p$data, common_essential==TRUE))
    }
  
  } else{
    p = p + labs(title=str_wrap(main, width=75))
  }
